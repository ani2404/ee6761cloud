...
flags.DEFINE_string("input", "/home/kps/Desktop/reference.png", "Directory name to save the checkpoints [checkpoint]")
flags.DEFINE_string("output", "/home/kps/Desktop/", "Directory name to save the checkpoints [checkpoint]")
...
class inference(object):
    def __init__(self,session,checkpoint_dir,image_size_x,image_size_y,resolution_factor=4):
        ...
        self.model = model.Model(session, checkpoint_dir=checkpoint_dir,batch_size=FLAGS.batch_size,
    ...
    def super_resolute(self,input_image):
        ...
        output_images,interpolated = self.session.run([self.model.ESCNN,self.model.interpolation],
                                        feed_dict={self.model.inputs:input_image})

        output_images = np.array(output_images).astype(np.float32)

        return output_images,interpolated
...
def main(_):
    ...
    if FLAGS.videoinput:
	    ...
	    downscaled = imresize(arr=image, size=[image.shape[0] / 4, image.shape[1] / 4])
            all_pixels = []
